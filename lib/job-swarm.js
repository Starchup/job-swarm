...
class JobSwarm
{
	...
	getNextJob(type, kind)
	{
		...
		kind = kind || this.config.entityKind;
	...
	{
		if (!this.config.controller) permissionError();
		if (!this.config.clearAfter || !this.config.clearAfter.trim()) throw new Error('getObsoleteJobs requires the controller to be instantiated with a clearAfter value');

		const [clearTime, clearUnit] = this.config.clearAfter.split(' ');
		if (!clearTime || !clearUnit) throw new Error(`config.clearAfter must be in format \`\$\{value} \$\{unit}\`. Actual: ${this.config.clearAfter}`);

		const cutoff = moment().subtract(clearTime, clearUnit);
		if (!cutoff.isValid()) throw new Error('Invalid date');
	...
	createJobs(data)
	{
		...
		const jobData = data.map(d => newOpenJob(d.name, d.type));
		return saveEntity.call(this, jobData);
	}
	...
	deleteJobs(filter)
	{
		...
		return this.getJobs(filter).then(jobs =>
		{
			...
			return this.datastore.delete(jobKeysToDelete).then(resultCount);
		});
	}
}
